...
def listar_produtos():
    ...
    descricao_parte = request.args.get('description_part', '').lower()

    produtos_filtrados = products

    #aqui é pra ordenar os produtos com base em suas descrições, e o usuario que vai escolher a descrição
    if descricao_parte:
        produtos_filtrados = [
            p for p in produtos_filtrados
            if descricao_parte in p.get("product_description", "").lower()
        ]

    #aqui é pra ordenar os produtos com base em seu preço do menor para o maior
    if preco_asc:
        produtos_filtrados = sorted(produtos_filtrados, key=lambda p: p["product_price"])
    elif preco_desc:
        produtos_filtrados = sorted(produtos_filtrados, key=lambda p: p["product_price"], reverse=True)
